export const m = {
  // SideMenu.vue
  Dashboard: 'Dashboard',
  General: 'General',
  User: 'User',
  Announcement: 'Announcement',
  System_Config: 'System Config',
  Judge_Server: 'Judge Server',
  Prune_Test_Case: 'Prune Test Case',
  FromFile: 'From File',
  ToFile: 'To File',
  ShareSubmission: 'Share Submission',
  Create_Problem: 'Create Problem',
  Export_Import_Problem: 'Export Or Import Problem',
  Quiz: 'Quiz',
  Quiz_List: 'Quiz List',
  Create_Quiz: 'Create Quiz',
  // Courses
  Courses: 'Courses',
  CoursesUser: 'Khoá học',
  Courses_List: 'Courses List',
  Create_Courses: 'Create Courses',
  Course_Result: 'Courses Result',
  Contest: 'Contest',
  Contest_List: 'Contest List',
  Create_Contest: 'Create Contest',
  AboutUs: 'Về chúng tôi',
  // User.vue
  User_User: 'Tài khoản',
  Import_User: 'Import User',
  Generate_User: 'Generate User',
  // User.vue-dialog
  User_Info: 'User',
  User_Username: 'Username',
  User_Real_Name: 'Họ tên',
  User_Email: 'Email',
  User_New_Password: 'New Password',
  User_Type: 'User Type',
  Problem_Permission: 'Problem Permission',
  Two_Factor_Auth: 'Two Factor Auth',
  Is_Disabled: 'Is Baned',
  User_Title: 'User Title',
  User_Title_Color: 'User Title Color',
  // Announcement.vue
  General_Announcement: 'Announcement',
  Announcement_Title: 'Title',
  Announcement_Content: 'Content',
  Announcement_visible: 'Visible',
  // Conf.vue
  SMTP_Config: 'SMTP Config',
  Server: 'Server',
  Port: 'Port',
  Email: 'Email',
  Password: 'Password',
  Website_Config: 'Web Config',
  Base_Url: 'Base Url',
  Name: 'Name',
  Shortcut: 'Shortcut',
  Footer: 'Footer',
  Allow_Register: 'Allow Register',
  Submission_List_Show_All: 'Submission List Show All',
  Sort: 'Chủ đề',
  Sort_Name: 'Sort Name',
  Sort_Permission: 'Sort Permission',
  Add_Sort: 'Add Sort',
  Allow_Forum: 'Open Forum',
  Allow_Reply: 'Allow Reply',
  Allow_Forum_FAQ: 'When closing, access to the forum is Redirected to 404 and Cannot post!',
  // JudgeServer.vue
  Judge_Server_Token: 'Judge Server Token',
  Judge_Server_Info: 'Judge Server',
  IP: 'IP',
  Judger_Version: 'Judger Version',
  Service_URL: 'Service URL',
  Last_Heartbeat: 'Last Heartbeat',
  Create_Time: 'Create Time',
  // PruneTestCase
  Test_Case_Prune_Test_Case: 'Prune Test Case',
  // Problem.vue
  Display_ID: 'Display ID',
  Title: 'Tiêu đề',
  Description: 'Đề bài',
  Input_Description: 'Dữ liệu vào',
  Output_Description: 'Dữ liệu ra',
  Time_Limit: 'Giới hạn thời gian',
  Memory_limit: 'Giới hạn bộ nhớ',
  Difficulty: 'Độ khó',
  Visible: 'Visible',
  Languages: 'Languages',
  Input_Samples: 'Input Samples',
  Output_Samples: 'Output Samples',
  Add_Sample: 'Add Sample',
  Code_Template: 'Code_Template',
  Special_Judge: 'Special Judge',
  Use_Special_Judge: 'Use Special Judge',
  Special_Judge_Code: 'Special Judge Code',
  SPJ_language: 'SPJ language',
  Compile: 'Compile',
  TestCase: 'TestCase',
  IOMode: 'Chế độ I/O',
  InputFileName: 'Input File Name',
  OutputFileName: 'Output File Name',
  Type: 'Type',
  Input: 'Dữ liệu vào',
  Output: 'Dữ liệu ra',
  Score: 'Điểm',
  Hint: 'Gợi ý',
  Source: 'Nguồn',
  Edit_Problem: 'Edit Problme',
  Add_Problme: 'Add Problem',
  High: 'Khó',
  Mid: 'Trung bình',
  Low: 'Dễ',
  Tag: 'Tag',
  New_Tag: 'New Tag',
  // ProblemList.vue
  Contest_Problem_List: 'Contest Problem List',
  // Contest.vue
  ContestTitle: 'Title',
  ContestDescription: 'Description',
  Contest_Start_Time: 'Start Time',
  Contest_End_Time: 'End Time',
  Contest_Password: 'Password',
  Contest_Rule_Type: 'Contest Rule Type',
  Real_Time_Rank: 'Real Time Rank',
  Contest_Status: 'Trạng thái',
  Allowed_IP_Ranges: 'Allowed IP Ranges',
  CIDR_Network: 'CIDR Network',
  // Courses.vue
  CoursesTitle: 'Title',
  CoursesName: 'Courses Name',
  CoursesAvatar: 'Avatar',
  CoursesDes: 'Description',
  CoursesSourse: 'Sources',
  NCC: 'NCC',
  SCORM: 'SCORM',
  CoursesLevel: 'Level',
  CoursesInfo: 'Thông tin khóa học',
  CoursesContent: 'Course content',
  Home: 'Home page',
  CoursesList: 'Course list',
  Basic: 'Basic',
  Advance: 'Advance',
  Expert: 'Expert',
  Recur: 'Recur',
  Perpetual: 'Perpetual',
  Course_StartTime: 'Start Time',
  Course_EndTime: 'End Time',
  Course_Visible: 'Visible',
  // section
  Section: 'Section',
  Section_List: 'Section List',
  Courses_Section_List: 'Courses Section List',
  Create_Section: 'Create Section',
  SectionName: 'Title',
  TotalQuizz: 'Total Quiz',
  Total_Time: 'Total Time',
  Released_Time: 'Released Time',
  Section_Visible: 'Visible',
  Courses_Visible: 'Visible',
  // Quiz.vue
  QuizTitle: 'Title',
  QuizDescription: 'Description',
  QuizTotalQuestion: 'Quiz Total Question',
  Quiz_Start_Time: 'Start Time',
  Quiz_End_Time: 'End Time',
  Quiz_Type: 'Quiz Type',
  // Question.vue
  Question: 'Question',
  Question_List: 'Question List',
  Create_Question: 'Create Question',
  // Dashboard.vue
  Last_Login: 'Last Login',
  System_Overview: 'System Overview',
  DashBoardJudge_Server: 'Judge Server',
  HTTPS_Status: 'HTTPS Status',
  Force_HTTPS: 'Force HTTPS',
  CDN_HOST: 'CDN HOST',
  // Login.vue
  Welcome_to_Login: 'Welcome to Login',
  GO: 'GO',
  username: 'username',
  password: 'password'
}
