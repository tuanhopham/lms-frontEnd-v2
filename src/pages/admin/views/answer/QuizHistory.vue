      ...
      >
        <div slot-scope="scope">
          <el-button
            ...
            style="width: 82px"
            :type="
              showAnswer && currentId === scope.row.quizUserId ? 'primary' : ''
            "
            @click="goAnswerList(scope.row.quizUserId)"
          >
            {{
              showAnswer && currentId === scope.row.quizUserId
                ? "Viewing"
                : "View"
            }}
          </el-button>
...
export default {
  ...
  methods: {
    goAnswerList(quizUserId) {
      ...
      if (this.showAnswer) {
        this.answerUser = this.quizListUser.find(
          i => i.quizUserId === quizUserId
        ).QA;
      } else {
        this.answerUser = [];
      }
