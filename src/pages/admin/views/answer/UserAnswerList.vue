      ...
      >
        ...
        <el-table-column label="Role" width="110" align="center">
          <template slot-scope="scope">
            <el-tag
              :type="
                scope.row.admin_type === 'Super Admin'
                  ? 'danger'
        ...
        <el-table-column label="Progress" width="140" align="center">
          <template slot-scope="scope">
            <div>
              <el-progress
                type="dashboard"
        ...
        <el-table-column fixed="right" width="220" label="Operation">
          <div slot-scope="scope">
            <icon-btn
              name="Edit"
              icon="edit"
              @click.native="goEdit(courseId, scope.row.userId)"
            />
            <icon-btn
              v-if="scope.row.status == 'Pending'"
              name="Completed"
              icon="check-square-o"
              @click.native="goCompleted(scope.row.userId)"
            />
          </div>
        </el-table-column>
      </el-table>
