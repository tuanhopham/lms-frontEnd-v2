<template>
    <Row type="flex" :gutter="18">
        ...
        <Panel shadow>
                ...
                <ul class="filter">
                    <li>
                        <Dropdown @on-click="filterByDifficulty" style="cursor:pointer;">
                            <span>{{ query.difficulty === '' ? this.$i18n.t('m.Difficulty') : this.$i18n.t('m.' +
                                query.difficulty) }}
            ...
            <Table style="width: 100%; font-size: 16px;" :columns="problemTableColumns" @on-sort-change="handleSortChange"
                :data="problemList" :loading="loadings.table" disabled-hover></Table>
        </Panel>
        ...
        <Panel :padding="10">
            ...
            <Button v-for="tag in tagList" :key="tag.name" @click="filterByTag(tag.name)" :disabled="query.tag === tag.name"
                shape="circle" class="tag-btn">{{ tag.name }}
            </Button>
        </Panel>
