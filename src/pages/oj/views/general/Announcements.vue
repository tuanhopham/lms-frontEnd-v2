<template>
    <Row type="flex" :gutter="18">
        ...
        <Panel shadow :padding="10">
            ...
            <div slot="extra">
                <Button v-if="listVisible" type="info" @click="init" :loading="btnLoading"><span
                        class="ivu-icon ivu-icon-ios-refresh"></span> {{ $t('m.Refresh') }}</Button>
            ...
            <transition-group name="announcement-animate" mode="in-out">
                ...
                <template v-if="listVisible">
                        ...
                        <li v-for="announcement in announcements" :key="announcement.title">
                            <div class="flex-container">
                                <div class="title"><a class="entry" @click="goAnnouncement(announcement)">
                                        <Icon type="md-bookmark" /> {{ announcement.title }}
                                    </a></div>
                    ...
                    <Pagination v-if="!isContest" key="page" :total="total" :page-size="limit"
        ...
        <Row v-if="!isContest" type="flex" :gutter="10" style="margin-top: 70px;">
            ...
            <Panel shadow style="padding-top: 0px;padding-bottom: 10px;">
                <div slot="title" style="margin-left: -10px;margin-bottom: -10px;">
                    <Icon type="md-document" /> Bài tập mới
                </div>
                <ul style="margin-left: 40px;margin-bottom: 20px;">
                    <li v-for="p in problemList" :key="p.id">
                        <a class="link-style liStyle" :href="'/problem/' + p._id">{{ p._id }} - {{ p.title }}</a>
                    </li>
                </ul>
            </Panel>
            </Col>
            ...
            <Panel shadow style="padding: 10px;padding-bottom: 10px;">
                <div slot="title" style="margin-left: -10px 0px 0px -20px;">
                    <Icon type="md-pricetags" /> {{ $t('m.TagsTitle') }}
                </div>
                <Button v-for="tag in tagList" :key="tag.name" :disabled="query.tag === tag.name" shape="circle"
                    class="tag-btn"><a class="link-style liStyle" :href="'/problem?tag=' + tag.name">{{ tag.name }}</a>
                </Button>
            </Panel>
            </Col>
        </Row>
        ...
        <Panel shadow>
            <div
                style="font-size:14px; text-align:center; width:100%; line-height:16px; background: transparent; color:#636e72;">
                Rủ bạn bè vào cày rank thôi nào!</div>
            <div class="today">
            ...
            <div v-if="days"
                style="margin:0 auto; margin-bottom:15px; font-size:12px; text-align:center; width:160px; line-height:16px; background: transparent; color:#636e72;">
                Bạn đã có <strong>{{ days }} </strong> chuỗi ngày học</div>
            <div
                style="margin-top:-10px; margin:0 auto; font-size:14px; text-align:justify; width:80%; line-height:16px; background: transparent; color:#636e72;">
                {{ word }}</div>
            <Button v-if="!SighinStatus" type="primary" icon="ios-alarm" @click="Sighin" long
                style="margin-top:20px; margin-bottom:20px; margin-left:10%; width:80%;">Ghi danh</Button>
            <Button v-else type="primary" icon="ios-alarm" long disabled
                style="margin-top:20px; margin-bottom:20px; margin-left:10%; width:80%;">
                Ghi danh
            </Button>
        </Panel>
        <Panel shadow style="margin-top: 37px;padding-bottom: 5px;">
            <div slot="title" style="margin-left: -10px;margin-bottom: -10px;">
                <Icon type="ios-link" /> {{ $t('m.Similar_Site') }}
            </div>
            <ul style="margin-left: 40px;margin-bottom: 20px;">
                <li><a href="" class="link-style liStyle"
                        onclick="event.preventDefault();window.open('https://oj.vnoi.info/?ref=luyencode', '_blank');">VNOJ:
                        VNOI Online Judge (🇻🇳)</a></li>
                <li><a href="" class="link-style liStyle"
                        onclick="event.preventDefault();window.open('https://freecontest.net/?ref=luyencode', '_blank');">Free
                        Contest (🇻🇳)</a></li>
                <li><a href="" class="link-style liStyle"
                        onclick="event.preventDefault();window.open('http://csloj.ddns.net/?ref=luyencode', '_blank');">Chuyên
                        Sơn La Online Judge (🇻🇳)</a></li>
                <li><a href="" class="link-style liStyle"
                        onclick="event.preventDefault();window.open('http://ntucoder.net/?ref=luyencode', '_blank');">NTUCoder
                        (🇻🇳)</a></li>
                <li><a href="" class="link-style liStyle"
                        onclick="event.preventDefault();window.open('https://codeforces.com/?ref=luyencode', '_blank');">Codeforces
                        (🇬🇧)</a></li>
                <li><a href="" class="link-style liStyle"
                        onclick="event.preventDefault();window.open('https://leetcode.com/?ref=luyencode', '_blank');">LeetCode
                        (🇬🇧)</a></li>
                <li><a href="" class="link-style liStyle"
                        onclick="event.preventDefault();window.open('https://discord.gg/hpeRrbccfZ', '_blank');">Luyện Code:
                        Hỏi & Đáp trên Discord (😘)</a></li>
                <li><a href="" class="link-style liStyle"
                        onclick="event.preventDefault();window.open('https://www.facebook.com/groups/LapTrinhKhongKho/', '_blank');">Nhóm
                        Lập Trình Không Khó (😘)</a></li>
                <li><a href="" class="link-style liStyle"
                        onclick="event.preventDefault();window.open('https://nguyenvanhieu.vn/', '_blank');">Blog học Lập
                        Trình Không Khó (😘)</a></li>
            </ul>
        </Panel>
        <Panel shadow style="margin-top: 37px;padding-bottom: 5px;">
            <div slot="title" style="margin-left: -10px;margin-bottom: -10px;">
                <Icon type="md-stats" /> {{ $t('m.Statistics') }}
            </div>
            <ul style="margin-left: 40px;margin-bottom: 20px;">
                <li><a href="" class="link-style liStyle" onclick="event.preventDefault();">Số lượng bài tập:
                        {{ problem_count }}</a></li>
                <li><a href="" class="link-style liStyle" onclick="event.preventDefault();">Tổng số bài nộp: overflow</a>
                </li>
                <li><a href="" class="link-style liStyle" onclick="event.preventDefault();">Số lượng thành viên:
                        {{ infoData.user_count }}</a></li>
                <li><a href="" class="link-style liStyle" onclick="event.preventDefault();">Số bài nộp hôm nay:
                        {{ infoData.today_submission_count }}</a></li>
            </ul>
        </Panel>
        </Col>
    </Row>
