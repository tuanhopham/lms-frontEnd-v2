<template>
    <Row v-if="website.allow_forum_post" type="flex" :gutter="18">
        ...
        <Panel shadow>
                ...
                <ul class="filter">
                    <li>
                        <Input v-model="query.keyword" @on-enter="filterByKeyword" @on-click="filterByKeyword"
                            placeholder="Từ khóa" icon="ios-search" />
                    </li>
                    <li>
            ...
            <Table style="width: 100%; font-size: 16px;" :columns="problemTableColumns" :data="ForumList"
        ...
        <Panel :padding="10">
            ...
            <br>
            <Button v-for="(sort, index) in website.forum_sort" :key="sort.name" @click="filterBySort(index + 1)"
                type="primary" ghost :disabled="query.sort === index + 1" shape="circle" class="tag-btn">{{ sort.name }}
            </Button>
        </Panel>
        <Spin v-if="loadings.tag" fix size="large"></Spin>
        <VerticalMenu @on-click="handleRoute" style="margin-top: 10px; cursor: pointer;">
            <VerticalMenu-item :disabled="contestMenuDisabled" :route="{ name: 'Forum-post' }" style="cursor: pointer;">
                    ...
                    render: (h, params) => {
                        ...
                        return h('div', [
                            h('Button', {
                                ...
                                on: {
                                    click: () => {
                                        this.$router.push({ name: 'Forum-details', params: { forumpostID: params.row.id } }).catch(() => { })
                                    }
                                },
                                style: light
                            }, params.row.title),
