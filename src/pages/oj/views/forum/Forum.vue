    ...
    <div v-if="website.allow_forum_post" class="flex-container">
        <div id="forumpost-main">
            ...
            <Panel :padding="40" shadow style="margin-top: 20px;">
                ...
                <div style="margin-top: -30px; margin-bottom: 30px">
                    <Card :padding="20" dis-hover v-for="reply in forumreplys" :key="reply.id" style="margin-top: 5px;">
                        <p slot="title">
                            <Tag v-if="reply.author.title" :color="reply.author.title_color" style="margin-top: -1px;">{{
                                reply.author.title }}</Tag>
                            <Tag v-else :color="user_grade[reply.author.grade].color" style="margin-top: -1px;">{{
                                user_grade[reply.author.grade].name }}</Tag>
                            <a @click="GoUserHome(reply.author.username)">
                                {{ reply.author.username }}
                            </a>
                        </p>
                        <p slot="extra">
                            <Button v-if="visible || reply.author.username === user.username" class="delete-Button"
                                type="text" @click="ConfirmDeleteForumReply = true, forumreplyID = reply.id">
                    ...
                    <Pagination :total="total" :page-size.sync="limit" :current.sync="page" @on-change="getForumReplyList"
                        show-sizer @on-page-size-change="getForumReplyList(1)">
                    </Pagination>
                </div>
            ...
            <Card :padding="20" id="submit-reply" dis-hover>
                ...
                <Row type="flex" justify="space-between">
                    ...
                    <Button type="warning" icon="edit" :loading="submitting" @click="submitForumReply" class="fl-right">
        ...
        <div id="right-column">
            ...
            <div class="VerticalMenu">
                <Card v-if="visible" :padding="0" dis-hover style="margin-top: 10px;cursor: pointer;">
                    <ul>
                        ...
                        <li v-if="isSuperAdmin">
                            ...
                            &nbsp;{{ $t('m.Forum_Top') }}
                            <i-switch v-model="forumpost.is_top" @on-change="ForumPostControl(2)"
                                style="margin-left: 10px;"></i-switch>
                        </li>
                        <li v-if="isSuperAdmin">
                            <Icon type="ios-bulb" style="margin-left: 2px;"></Icon>
                            &nbsp;{{ $t('m.Forum_Light') }}
                            <i-switch v-model="forumpost.is_light" @on-change="ForumPostControl(3)"
                                style="margin-left: 10px;"></i-switch>
                        </li>
                        <li v-if="isSuperAdmin">
                            <Icon type="md-star" style="margin-left: -1px;"></Icon>
                            {{ $t('m.Forum_Nice') }}
                            <i-switch v-model="forumpost.is_nice" @on-change="ForumPostControl(4)"
                                style="margin-left: 10px;"></i-switch>
                        </li>
        ...
        <Modal v-model="ConfirmDeleteForumPost" :title="$t('m.Forum_Delete')" @on-ok="confirmDeletePost">
            <p>{{ $t('m.Forum_ConfirmDelete') }}</p>
        </Modal>
        <Modal v-model="ConfirmDeleteForumReply" :title="$t('m.Forum_Delete')" @on-ok="confirmDeleteReply">
            <p>{{ $t('m.Forum_ConfirmDelete') }}</p>
        </Modal>
    </div>
        ...
        <div class="error404-body-con">
            <Card>
                ...
                <div class="error404-btn-con">
                    ...
                    <Button @click="backPage" size="large" style="width: 200px;margin-left: 40px;"
                        type="primary">{{ $t('m.Back') }}</Button>
                </div>
            </Card>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        ForumPostControl(method) {
            if (method === 1) {
                // Edit
                this.$router.push({ name: 'Forum-post', query: { id: this.forumpostID } }).catch(() => { })
            }
            if (method === 2) {
                // Top
        ...
        confirmDeletePost() {
            api.deleteFourmPost(this.forumpostID).then(res => {
                ...
                this.$router.push({ name: 'Forum-list' }).catch(() => { })
            })
        },
